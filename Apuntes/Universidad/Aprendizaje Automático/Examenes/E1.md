...
B) En el caso de que decidan  aproximarlo con aprendizaje:
	B1) Establezca de forma clara y precisa la estructura del espacio de etiquetas _Y_ y la función _f_ que etiqueta las piezas.
...
b) En caso afirmativo, decir cuando se produce dicho evento, bajo que cirscunstancias se produce  y como sabemos que hemos fallado.
...
La desigualdad $P[|E_{out}-E_{in}|>\epsilon(N,d_{vc})] \leq \delta(N, d_{vc})$  nos sirve para analizar cuando se produce. Los valores  de las constantes $\epsilon$ y $\delta$ dependen del tamaño muestral y de la clase de funciones. 
...
Si analizamos el caso  k=1 vemos que el punto de ruptura es 3 ya que solo podemos separar dos particiones, los casos (x o, o x) . k=2 el punto de ruptura es 5 ( oxoxo , xoxox). k=3 el punto de ruptura es 7, etc. Entonces por inducción 2k+1
